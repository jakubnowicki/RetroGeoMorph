minimal.distance.matrix.iter.half <- function(data, Csize=FALSE,curve) {
  ...
  n.pairs<-dim(pairs)[2]
  wynik<-NULL
  wynik =  foreach(i=1:n.pairs,.export = c('fpdist','iterative.minimal.distance.seeker.gpg','min.dist.seeker.gpg','strain.matrix.comp','deformacja'),.packages = c('shapes','abind','geomorph','dplyr')) %dopar% {
